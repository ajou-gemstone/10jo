...
public class FCFSAdapter extends RecyclerView.Adapter<FCFSAdapter.ViewHolder> {
    ...
    public FCFSAdapter(ArrayList<String> list) {
        mData = list ;
    }

    // onCreateViewHolder() - 아이템 뷰를 위한 뷰홀더 객체 생성하여 리턴.
    @Override
    public FCFSAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        Context context = parent.getContext() ;
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) ;

        View view = inflater.inflate(R.layout.recyclerview_lectureroom_reservation_fcfs, parent, false) ;
        FCFSAdapter.ViewHolder vh = new FCFSAdapter.ViewHolder(view) ;

        return vh ;
    }

    // onBindViewHolder() - position에 해당하는 데이터를 뷰홀더의 아이템뷰에 표시. 표시만 정의해주고 click같은 기능은 viewholder에서 처리해준다.
    @Override
    public void onBindViewHolder(FCFSAdapter.ViewHolder holder, int position) {
